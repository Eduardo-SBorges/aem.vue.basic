...
This project was bootstrapped with [`vue create`](https://cli.vuejs.org/guide/creating-a-project.html).

This application is built to consume the AEM model of a site. It will automatically generate the layout using the helper components from the [`@mavice/aem-vue-editable-components`](https://www.npmjs.com/package/@mavice/aem-vue-editable-components) package.
...
After running `npm run serve` **in the `ui.frontend` directory**, your app will be automatically opened in your browser (at path http://localhost:8000/content/vue/home.html). If you make edits, the page will reload.
...
Launches the test runner in the interactive watch mode. See the section about [running tests](https://facebook.github.io/create-Vue-app/docs/running-tests) for more information.

### `npm run build`

Builds the app for production to the `build` folder. It bundles Vue in production mode and optimizes the build for the best performance. See the section about [deployment](https://facebook.github.io/create-Vue-app/docs/deployment) for more information.
...
The Vue app is configured to make use of [code splitting](https://webpack.js.org/guides/code-splitting) by default. When building the app for production, the code will be output in several chunks:
